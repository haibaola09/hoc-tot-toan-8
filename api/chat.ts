...
// Ensure the API key is read properly
  ...
  try {
    ...
    try {
      ...
      if (errStr.includes("503") || errStr.includes("unavailable") || errStr.includes("429") || errStr.includes("demand") || errStr.includes("limit")) {
        return res.json({
          reply: `**Cám ơn câu hỏi rất hay của em!**\n\nHiện tại hệ thống trí tuệ nhân tạo (Gemini API) của máy chủ Thầy Hải đang gặp phải tình trạng quá tải tạm thời. Em hãy nghỉ ngơi hoặc uống nước tầm **10-15 giây** rồi gửi lại câu hỏi cho Thầy nhé. Thầy luôn sẵn lòng hướng dẫn và đồng hành học toán cùng em! 😊`
        });
      }
